...
class EmbeddingBlock(layers.Layer):
    def __init__(self, emb_size, activation=None,
        ...
        self.atom_init = tf.constant(np.load('D:/OneDrive/OneDrive - zju.edu.cn/桌面/C2DB文章/model/data/atom_init.npy'), dtype=tf.float32)
        self.dense_embeddings = layers.Dense(self.emb_size, activation=activation, use_bias=True,
    ...
    def call(self, inputs):
        ...
        embeddings = self.dense_embeddings(self.atom_init)

        Z_i = tf.gather(Z, idnb_i)
        Z_j = tf.gather(Z, idnb_j)

        x_i = tf.gather(embeddings, Z_i)
        x_j = tf.gather(embeddings, Z_j)
